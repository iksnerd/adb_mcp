...
React Native/Expo dev-client app across several long debugging sessions.

- [ ] **`screenshot` should signal WHY a capture is black.** Two different black-image cases both returned a bare black PNG with no explanation, causing repeated misdiagnosis: (a) an intermittent silent black capture on *normal* content that succeeded on retry (looked like "screen asleep" but wasn't) → add a retry-on-black or a low-confidence flag; (b) `FLAG_SECURE` windows (e.g. the native PIN pad) are blanked black by the OS → detect and say so. Ask: have the `screenshot` result carry a small status field (`{ ok, allBlack, secureWindow?, screenOff? }`) so the agent doesn't guess. Documented workaround that already works: when a screenshot is black, fall back to `describe_ui` (it returned the full hierarchy even for the secure PIN pad). This is the highest-value screenshot item.
- [ ] **`last_crash` crash helper** (also tracked under XcodeBuildMCP parity gaps above) — the real fatals came from DropBox `data_app_crash`, with the `FATAL EXCEPTION` header sometimes separated from the frames in filtered views. Pull `dumpsys dropbox --print data_app_crash` / tombstone so the whole stack comes back in one call.
