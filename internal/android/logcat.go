...
func Logcat(ctx context.Context, serial string, lines int, since string, f LogFilter) (string, error) {
	if err := f.validate(); err != nil {
		return "", err
	}
	window := ""
	if strings.TrimSpace(since) != "" {
		cutoff, err := deviceCutoff(ctx, serial, since)
		if err != nil {
			return "", err
		}
		window = cutoff
	...
	out, err := runAdb(ctx, serial, "logcat", "-d", "-v", "threadtime", "-t", window)
	if err != nil {
		return "", err
	}
...
func ClearLogcat(ctx context.Context, serial string) error {
	_, err := runAdb(ctx, serial, "logcat", "-c")
	return err
}
...
func deviceCutoff(ctx context.Context, serial, since string) (string, error) {
	now, err := runAdb(ctx, serial, "shell", "date", "+%s %z")
	if err != nil {
